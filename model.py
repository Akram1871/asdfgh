...
#                                   WarmupLinearSchedule,BertModel,BertPreTrainedModel,AutoModel,AutoTokenizer,AutoModelWithLMHead)
from pytorch_transformers import (AdamW,
                                  WarmupLinearSchedule)
from transformers import (AutoModel,AutoTokenizer)
from torch import nn, randint_like
...
class Ner(nn.Module):
    ...
    def __init__(self,
            ...
            rnn_type: str = "LSTM",
            # GAT parameters
            nhid: int = 128, gat_dropout: float = 0.7, alpha: float = 0.3, nheads: int = 4
        ...
        if self.use_gat:
            ...
            #     GAT Before LSTM
            #---------------------------------------------------------------------
            self.gat_dropout1 = gat_dropout*1.2
            self.nhid1 = 192
            self.alpha1 = alpha
            self.nheads1 = nheads
            ...
            if 'A' in self.gat_type:
                self.gat_a1 = GAT(
                    nfeat=rnn_input_dim,
                    nhid=self.nhid1,
                    dropout=self.gat_dropout1,
                    nheads=self.nheads1,
                    alpha=self.alpha1, nclass=self.tagset_size)
                if self.fuse_type == 'v':
                    self.weight21 = torch.nn.Parameter(torch.ones(self.nhid1 * self.nheads1))
                elif self.fuse_type == 'n':
                    self.weight21 = torch.nn.Parameter(torch.ones(1))
            ...
            if 'F' in self.gat_type:
                self.gat_f1 = GAT(
                    ...
                    alpha=self.alpha1, nclass=self.tagset_size)
                if self.fuse_type == 'v':
                    self.weight31 = torch.nn.Parameter(torch.ones(self.nhid1 * self.nheads1))
                elif self.fuse_type == 'n':
                    self.weight31 = torch.nn.Parameter(torch.ones(1))

            if self.fuse_type == 'v':
                self.weight11 = torch.nn.Parameter(torch.ones(rnn_input_dim))
            elif self.fuse_type == 'n':
                self.weight11 = torch.nn.Parameter(torch.ones(1))
        ...
        if self.use_gcn:
            ...
            self.gcn_dropout = gat_dropout
            self.nhid = nhid
            #self.alpha = alpha
            #self.nheads = nheads
            
            self.gcn_a = GCN(
                node_features=hidden_size * 2,
                hidden_dim=self.nhid,
                num_classes=self.tagset_size,
        ...
        if self.use_gat:
            if self.fuse_type == 'm':
                ...
                #     (len(self.gat_type)+1)*self.nheads1 * self.nhid1, self.output_size
                # )
            else:
                self.linear = torch.nn.Linear(
    ...
    def forward(self,  input_ids, token_type_ids=None, attention_mask=None, labels=None,valid_ids=None,attention_mask_label=None, adj_a=None, adj_f=None):
        ...
        # print(f'\nBert shape = {sequence_output.shape}')
        lengths = []
        ...
        if self.use_gat:
            ...
            if 'A' in self.gat_type:
                ...
                sequence_output_a = self.gat_a1(sequence_output, adj_a)
            if 'F' in self.gat_type:
                assert (adj_f is not None)
                sequence_output_f = self.gat_f1(sequence_output, adj_f)

            if self.fuse_type == 'v':
                if self.gat_type == 'AF':
                    sequence_output = torch.mul(sequence_output, self.weight11) + torch.mul(sequence_output_a,
                                                                                           self.weight21) + torch.mul(
                        sequence_output_f, self.weight31)
                elif self.gat_type == 'A':
                    sequence_output = torch.mul(sequence_output, self.weight11) + torch.mul(sequence_output_a,
                                                                                           self.weight21)
                elif self.gat_type == 'F':
                    sequence_output = torch.mul(sequence_output, self.weight11) + torch.mul(sequence_output_f,
                                                                                           self.weight31)

            # n strategy
            if self.fuse_type == 'n':
                if self.gat_type == 'AF':
                    sequence_output = self.weight11 * sequence_output + self.weight21 * sequence_output_a + self.weight31 * sequence_output_f
                elif self.gat_type == 'A':
                    sequence_output = self.weight11 * sequence_output + self.weight21 * sequence_output_a
                elif self.gat_type == 'F':
                    sequence_output = self.weight11 * sequence_output + self.weight31 * sequence_output_f
        ...
        # print(f'\n After GAT before lstm shape = {sequence_output.shape}')
        if self.use_rnn:
        ...
        # print(f'\n After lstm shape = {sequence_output.shape}')
        if self.use_gat:
            # # --------------------------------------------------------------------
            # # GAT after BiLSTM
            # # --------------------------------------------------------------------
            ...
            if self.fuse_type == 'm':
                if self.gat_type == 'AF':
                    ...
                    sequence_output = torch.cat((sequence_output, sequence_output_f), dim=2)
        # print(f'\n After GAT 2 shape = {sequence_output.shape}')
        # print(f'\n After GAT 2 w1 = {self.weight1.shape}')
        # print(f'\n After GAT 2 w2 = {self.weight2.shape}')
        # print(f'\n After GAT 2 w3 = {self.weight3.shape}')
        if self.use_gcn:
          # # --------------------------------------------------------------------
          # # GCN after BiLSTM
          # # --------------------------------------------------------------------
          ...
          sequence_output = self.gcn_a(sequence_output, adj)
