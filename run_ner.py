...
# from pytorch_transformers import (WEIGHTS_NAME, AdamW, AutoConfig, BertConfig,
#                                   BertForTokenClassification, BertTokenizer,
#                                   WarmupLinearSchedule,BertModel,BertPreTrainedModel,AutoModel,AutoTokenizer)
# from pytorch_transformers import (WEIGHTS_NAME, AdamW, WarmupLinearSchedule)
from transformers import (AdamW,AutoModel,AutoTokenizer,AutoConfig,get_linear_schedule_with_warmup)
...
def main():
    ...
    model = Ner(args.bert_model,batch_size=args.train_batch_size,from_tf = False, config = config,tag_to_ix=tag2ix, device=device, use_rnn=args.use_rnn, use_crf=args.use_crf, use_gat=args.use_gat,use_gcn=args.use_gcn, gat_type=args.gat_type,fuse_type=args.fuse_type, reproject_embeddings=args.relearn_embed)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                elif args.use_gcn and args.gat_type == 'AF':
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a, adj_f = batch
                elif args.use_gcn and args.gat_type == 'A':
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a = batch
                elif args.use_gcn and args.gat_type == 'F':
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_f = batch
                else:
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask = batch
    ...
    if args.do_predict:
        ...
        config = AutoConfig.from_pretrained(args.bert_model, num_labels=num_labels, finetuning_task=args.task_name)
        tokenizer = AutoTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
        model = Ner(args.bert_model,batch_size=args.eval_batch_size, from_tf=False, config=config, tag_to_ix=tag2ix, device=device, use_rnn=args.use_rnn, use_crf=args.use_crf, use_gat=args.use_gat,use_gcn=args.use_gcn,gat_type=args.gat_type,fuse_type=args.fuse_type,reproject_embeddings=args.relearn_embed)
        model.to(device)
        if n_gpu > 1:
            model = torch.nn.DataParallel(model)
        if args.local_rank != -1:
